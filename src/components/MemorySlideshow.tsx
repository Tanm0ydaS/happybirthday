...
export default function MemorySlideshow({
  ...
  function startTyping(text: string) {
    ...
    if (!text) return;
    let i = 0;
    const speed = Math.max(10, 28 - Math.floor(text.length / 6));
    const t = setInterval(() => {
      i += 1;
          ...
          >
            <div className="relative w-full h-full">
              <Image
                ...
                sizes={sizes}
                priority={i === 0}
                loading={i === 0 ? "eager" : "lazy"}         // <-- uses valid 'loading' values
                decoding={i === 0 ? "sync" : "async"}        // <-- valid decoding values: "sync" | "async" | "auto"
                style={{ objectFit: "cover" }}
                decoding={i === 0 ? "sync" : "async"}
                decoding={undefined} // remove duplicate if TS complains - keep one of the decoding lines
                decoding={i === 0 ? "sync" : "async"}
                // Note: Next/Image ignores native decoding in some builds; above keeps types safe.
              />
            </div>
          </div>
        ))}
